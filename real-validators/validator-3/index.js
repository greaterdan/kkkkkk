...
class EmbeddingsValidator {
  constructor() {
    this.app = express();
    this.port = process.env.PORT || 8554;
    this.validatorId = 'embeddings-validator-003';
    this.validatorAddress = '0x6436d4fd3c71c0532925a3b8D4C9db96C4b4d3c71';
    this.validatorName = 'Embeddings Pro Validator';
    this.stakeAmount = BigInt('20000000000000000000000'); // 20,000 01A
    this.commission = 3.0; // 3%
    this.subnetId = 'subnet-3'; // Embeddings Pro subnet
    ...
    this.db = new Database('embeddings_validator.db');
    this.initDatabase();
  ...
  setupWebSocket() {
    this.wss = new WebSocketServer({ port: 8555 });
    this.wss.on('connection', ws => {
      console.log('Embeddings Validator WebSocket client connected');
    });
  }
  ...
  async monitorL2Network() {
    try {
      ...
      console.log(`🔗 Embeddings Validator connected to L2 network. Current block: ${blockNumber}`);
      
      this.provider.on('block', (blockNumber) => {
        console.log(`📦 Embeddings Validator detected new block: ${blockNumber}`);
        this.lastBlockTime = Date.now();
      ...
      console.error('❌ Embeddings Validator failed to connect to L2 network:', error);
  ...
  async validateBlock() {
    try {
      ...
      if (validationSuccess) {
        ...
        console.log(`✅ Embeddings Validator validated block successfully. Reward: ${ethers.formatEther(reward)} 01A`);
        ...
        console.log(`❌ Embeddings Validator block validation failed`);
      ...
      console.error('❌ Embeddings Validator validation error:', error);
  ...
  startValidator() {
    this.app.listen(this.port, () => {
      console.log(`🚀 Embeddings Validator running on port ${this.port}`);
    ...
    console.log(`🌐 Embeddings Validator WebSocket server running on port 8555`);
  }
}
