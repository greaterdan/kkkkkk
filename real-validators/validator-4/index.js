...
class AudioValidator {
  constructor() {
    this.app = express();
    this.port = process.env.PORT || 8556;
    this.validatorId = 'audio-validator-004';
    this.validatorAddress = '0xf405774504c9c0532925a3b8D4C9db96C4b4d04c9';
    this.validatorName = 'Audio Genesis Validator';
    this.stakeAmount = BigInt('25000000000000000000000'); // 25,000 01A
    this.commission = 4.0; // 4%
    this.subnetId = 'subnet-4'; // Audio Genesis subnet
    ...
    this.db = new Database('audio_validator.db');
    this.initDatabase();
  ...
  setupWebSocket() {
    this.wss = new WebSocketServer({ port: 8557 });
    this.wss.on('connection', ws => {
      console.log('Audio Validator WebSocket client connected');
    });
  }
  ...
  async monitorL2Network() {
    try {
      ...
      console.log(`🔗 Audio Validator connected to L2 network. Current block: ${blockNumber}`);
      
      this.provider.on('block', (blockNumber) => {
        console.log(`📦 Audio Validator detected new block: ${blockNumber}`);
        this.lastBlockTime = Date.now();
      ...
      console.error('❌ Audio Validator failed to connect to L2 network:', error);
  ...
  async validateBlock() {
    try {
      ...
      if (validationSuccess) {
        ...
        console.log(`✅ Audio Validator validated block successfully. Reward: ${ethers.formatEther(reward)} 01A`);
        ...
        console.log(`❌ Audio Validator block validation failed`);
      ...
      console.error('❌ Audio Validator validation error:', error);
  ...
  startValidator() {
    this.app.listen(this.port, () => {
      console.log(`🚀 Audio Validator running on port ${this.port}`);
    ...
    console.log(`🌐 Audio Validator WebSocket server running on port 8557`);
  }
}
